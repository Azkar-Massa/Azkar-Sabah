...
function updateZikr() {
    ...
    // يقرأ من فولدر audio2 وملفات Morning الكابيتال
    const audioUrl = "audio2/Morning" + (currentIndex + 1) + ".mp3";
    currentAudio = new Audio(audioUrl);

    // تشغيل الصوت تلقائياً عند الانتقال للذكر التالي بشرط أن يكون المستخدم قد لمس الشاشة مرة واحدة على الأقل
    if (hasInteracted && currentAudio) {
        currentAudio.play().catch(e => console.log("خطأ في التشغيل التلقائي"));
    }
}
...
function enableAutoplay() {
    if (!hasInteracted) {
        hasInteracted = true;
        if (currentAudio) {
            currentAudio.play().catch(e => console.log("تم السماح بالصوت بنجاح"));
        }
        // إزالة مستمع اللمس بعد تفعيله لأول مرة
        document.removeEventListener("click", enableAutoplay);
        document.removeEventListener("touchstart", enableAutoplay);
    }
}

// الاستماع لأول لمسة على الشاشة لتفعيل نظام الصوت التلقائي
document.addEventListener("click", enableAutoplay);
document.addEventListener("touchstart", enableAutoplay);

playAudioBtn.addEventListener("click", () => {
    if (currentAudio) {
        currentAudio.play().catch(err => console.log("المتصفح حظر الصوت"));
    }
});
